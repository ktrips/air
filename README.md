...
- 🔗 **旅行記の短縮共有リンク**: `https://air.ktrips.net/?s=Ab3xK9` の形式（6文字コード）で旅行記を共有。共有ボタン（𝕏 / 📷 / Facebook / 🔗コピー）を開くとトリップの持ち主側でコードが自動発行され（Firestore の `shortCode`）、リンクを開くと該当トリップの旅行記が自動で表示される。**公開トリップ**なら未ログインの外部ユーザーも閲覧可（非公開は持ち主のみ）
- ⚡ **初期表示の高速化**: `exifr`（EXIF読み取り）を写真アップロード時の初回のみ遅延読み込み。Firebase / Firestore / Leaflet CDN への `preconnect` を追加
