...
- 🎬 **動画コントロールバー**: 動画表示中に画面上中央へ ⏮前 ⏸/▶再生停止 ⏭次 ✕終了 ボタンを表示
- ▶️ **YouTube Shorts 対応**: `youtube.com/shorts/` 形式のURLも自動再生に対応
- ⏭ **動画の自動連続再生**: 動画終了時に自動で次の動画へ移動
- 📐 **動画表示スペース拡大**: 画面の70%を動画に使用（従来比+20%）
- 📸 **自動再生改善**: スライドショー中の写真がやや大きく表示。再生終了後はスタンプ一覧なしで地図に復帰
...
- **ルートカラー**: 15色からトリップカラーを選択（ピンク〜シアンのレインボー順＋緑系）
...
### 機能
- **親子トリップ**: 親トリップはフォルダとして機能し、写真やGPXは持ちません。親トリップ選択時は全子トリップのアニメサムネイルをまとめてインライン表示し、旅行記一覧では各子トリップに📖旅行記・🎬動画ボタンを表示します。動画再生は子トリップ・ポイントの動画を連続再生します
- **自動再生（スライドショー）**: 再生中は写真がやや大きく表示。再生終了後は地図全体表示に戻ります（スタンプ一覧は表示されません）
