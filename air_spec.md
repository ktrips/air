...
- **閲覧**: ログイン不要で誰でも閲覧可能
- **短縮共有リンク**: `?s=<shortCode>`（例: `https://air.ktrips.net/?s=Ab3xK9`）
  - `shortCode`: トリップドキュメントの6文字コード（紛らわしい `0/O/1/l/I` を除外）。旅行記の共有ボタン表示時に、持ち主がアクセスすると未発行の場合のみ自動発行して Firestore に保存
  - 解決: 自分のトリップ → `where('shortCode','==',code).where('public','==',true)` の順で検索し、該当トリップを読み込んで旅行記モーダルを自動で開く
  - 非公開トリップの短縮リンクは持ち主のみ閲覧可。パス形式（`/t/xxx`）は `index.html` が相対パスでアセットを読むため未採用
